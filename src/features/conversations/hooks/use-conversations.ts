...
export const useConversation = (id: Id<"conversations"> | null) => {
  return useQuery(api.conversations.getConversationById, id ? { conversationId: id } : "skip");
};
...
export const useConversations = (projectId: Id<"projects">) => {
  return useQuery(api.conversations.getConversationsByProjectId, { projectId });
};
